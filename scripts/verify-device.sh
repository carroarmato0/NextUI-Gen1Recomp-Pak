...
adb push "$SMOKE_LOVE" "$ROMDIR/_smoke.love" >/dev/null 2>&1 \
...
adb shell "rm -f '$ROMDIR/_smoke.love'" >/dev/null 2>&1
...
if [ -z "$GLOG" ]; then
    ...
    if printf '%s\n' "$GLOG" | matches 'rom .*matched (Red|Blue|Yellow)'; then
        ok "a cartridge dump was found and imported by SHA-1"
        ...
        warn "no ROM matched -- put a US Red/Blue/Yellow dump in Roms/Game Boy (GB) to test the import path"
    else
    ...
    adb shell "ls '$SD/Emus/$PLATFORM/Gen1Recomp.pak/game/' 2>/dev/null" | matches 'portable.txt' \
