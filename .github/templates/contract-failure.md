...
- **ROM discovery moved.** `findPendingRom` is gone, or it no longer scans the
  PhysFS root, or `baseRomDiscovery` stopped being Xbox-only. We stage dumps at the
  save-dir root precisely because that is the only place Linux looks; if that
  changes, the pak copies a dump somewhere the engine never reads and the player
  just sees "no ROM".
...
- `POKEPORT_GBCFX` is no longer honoured, so GBC FX could render black frames on
  PowerVR/Mali.
- **The canonical ROM SHA-1s no longer appear in the payload**, i.e. the engine
  accepts a different set of dumps. Our scan matches on SHA-256 (the device has no
  `sha1sum`), so the two lists have to be kept in step by hand in `upstream.lock`.
